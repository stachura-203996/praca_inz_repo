...
`<selector>` must be an array of objects that will receive new `RoleGuard`s (dispatch
positions) for the method. If an object in the selector is a function, its
`.prototype` field will receive the new `RoleGuard`. The array must not contain any
frozen objects.

When a `genfun` is called (like a function), it will look at its set of added
methods and, based on the `RoleGuard`s assigned, and corresponding prototype chains,
